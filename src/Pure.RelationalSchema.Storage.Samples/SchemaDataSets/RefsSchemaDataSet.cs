...
// The refs domain: one relation, referenced from schema_with_foreign_keys on a
// string key and declaring no foreign key of its own. It is the referenced side
// of the only outbound cross-schema edge, and holds one row no order points at.
public sealed record RefsSchemaDataSet : IStoredSchemaDataSet
{
    public ISchema Schema => new RefsRelationalSchema();
