...
**Package validation** is enabled in the csproj against `PackageValidationBaselineVersion` 0.1.0-preview.1.0.0, so removing or renaming a public type fails the build. Sample *contents* may change freely — that is a behavioural break, documented in the changelog, not an API one. Bump the baseline when a new version is published.
